...
def main():
    ...
    print("Size of each batch: " + str(batch_size))
    ...
    network_model = MyImg2Num(input_dim, hidden_dims, output_dim)
    ...
    while epoch_num < max_epochs:
        ...
        for current_batch in range(num_batches-1):
            y_data_batch_template = torch.zeros(batch_size,output_dim)

            batch_ix, (x_data, y_data) = next(samples)
            print("batch num: "+str(batch_ix)+"/"+str(num_batches)+"\t \
                epoch num: "+str(epoch_num+1)+"/"+str(max_epochs))

            current_batch_y = y_data_batch_template
            for row_ix, row in enumerate(current_batch_y):
                current_batch_y[row_ix][int(y_data[row_ix])] = 1

            x_data = x_data.view(x_data.shape[0], input_dim).float()

            #initialize this better

            current_batch_x = x_data
            ...
            for loss in mean_losses:
                #if batch_ix % 100 == 0:
                    #print("Batch: " + str(batch_ix) + "\tLoss: " + str(loss))
                #print(loss)
...
def show_image_batch(x_batch, y_batch, network_model, input_dim):

    num_images = len(x_batch[:,0])
    print(num_images)
    ...
    for row_ix in range(1,num_images+1):
        numpy_array = x_batch[row_ix-1,:].squeeze().numpy()
        fig.add_subplot(num_rows, num_cols, row_ix)
        plt.imshow(numpy_array, cmap='Greys')
        plt.axis('off')
        plt.grid(False)

    x_batch = x_batch.view(x_batch.shape[0], input_dim).float()
    guess_results = network_model.forward(x_batch)
    print(y_batch)
    print(guess_results)
